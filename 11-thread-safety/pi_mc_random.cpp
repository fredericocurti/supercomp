...
Usage:
   ...
   and change the value of TRIALS to change the number of samples
...
int main() {
    ...
    int numbers_size = omp_get_max_threads() * TRIALS;
    double *numbers_x = new double[numbers_size];
    double *numbers_y = new double[numbers_size];
    int index = 0;

    double time = omp_get_wtime();

    for (i = 0; i < numbers_size; i++) {
        numbers_x[i] = distribution(generator);
        numbers_y[i] = distribution(generator);
    }

    #pragma omp parallel for private(test, i) reduction(+:Ncirc)
    for (i = 0; i < TRIALS; i++) {
        index = (omp_get_thread_num() * TRIALS) + i;
        test = numbers_x[index] * numbers_x[index] + numbers_y[index] * numbers_y[index];
        if (test <= r * r) {
            Ncirc++;
        }
    }

    pi = 4.0 * ((double)Ncirc / (double)TRIALS);

    // printf("Ncirc %ld \n", Ncirc);
    printf("\n %ld trials, pi is %lf ", TRIALS, pi);
